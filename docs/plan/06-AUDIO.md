...
**The beats.** Simple is generated by rule, so it fits any signature. Every other beat is
**drum tab** in `src/domain/drums/beats.ts` — a line per drum, `|` between bars, `- g x X`
for rest, ghost, hit and accent — parsed by `drumTab()`, which throws on a typo so the unit
...
when to start. A beat that does not fit the phrase's signature — or no longer exists —
plays Simple. `M` toggles
