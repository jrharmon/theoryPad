...
const ALL_KEYS = MODE_NAMES.flatMap((mode) =>
  ALL_CHROMAS.map((c) => ({ tonic: preferredTonic(c, mode), mode })),
);
...
describe('scaleNotes', () => {
  ...
  it.each([
    ...
    expect(scaleNotes({ tonic: pitchClass(tonic), mode }).join(' ')).toBe(expected);
  });
});
...
describe('scaleDegrees', () => {
  it.each([
    ...
    const degrees = scaleDegrees({ tonic: pitchClass('C'), mode });
    expect(degrees.map((d) => d.label).join(' ')).toBe(expected);
  });

  it('gives the same formula regardless of tonic', () => {
    for (const mode of MODE_NAMES) {
      const reference = scaleDegrees({ tonic: preferredTonic(0 as Chroma, mode), mode }).map(
        (d) => d.label,
      );
...
describe('degreeOf', () => {
  ...
  it('matches by pitch, not by spelling', () => {
    const km = { tonic: pitchClass('Eb'), mode: 'ionian' as const };
...
describe('signature degree', () => {
  ...
  it.each([
    ...
    const km = { tonic: pitchClass('D'), mode };
