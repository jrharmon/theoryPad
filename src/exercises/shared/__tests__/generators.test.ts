...
describe('intervalRun', () => {
  ...
  it('opens a descent on a descending figure, whichever the pairing', () => {
    const same = intervalRun({ positions: EIGHT, pattern: pattern('3rds'), pairing: 'same-direction', direction: 'descending' });
    expect(same.slice(0, 4)).toEqual([8, 6, 7, 5]);
    const alt = intervalRun({ positions: EIGHT, pattern: pattern('3rds'), pairing: 'alternating', direction: 'descending' });
    expect(alt.slice(0, 4)).toEqual([8, 6, 5, 7]);
  });

  it('does not repeat the figure it turns on', () => {
    const run = intervalRun({ positions: [1, 2, 3, 4], pattern: pattern('3rds'), pairing: 'same-direction', direction: 'up-down' });
...
describe('arpeggioRun', () => {
  ...
  it.each(GUITARS)('keeps only chord tones, in the shape’s order (%s)', (_id, instrument) => {
    const shape = scaleRun({ instrument, keyMode: D_DORIAN, direction: 'ascending', minFret: 5, startDegree: 1 });
...
describe('oneNotePerString', () => {
  ...
  it.each(GUITARS)('plays one note per string, walking the scale (%s)', (_id, instrument) => {
    ...
    const notes = oneNotePerString({ instrument, keyMode: D_DORIAN, strings, step: 2, stop: { kind: 'return-to-root' } });
  ...
  it('stops after a fixed number of sweeps, back on the starting string', () => {
    const notes = oneNotePerString({ instrument: STANDARD_GUITAR, keyMode: C_MAJOR, strings: [2, 3, 4], step: 1, stop: { kind: 'cycles', count: 2 } });
...
describe('shapeFrom', () => {
  ...
  it.each(GUITARS)('moves down to fit near the top of the neck (%s)', (_id, instrument) => {
    ...
    expect(Math.max(...shape.positions.map((p) => p.fret))).toBeLessThanOrEqual(instrument.fretCount);
  });
});
...
describe('horizontalRun', () => {
  ...
  function check(instrument: Instrument, shiftOn: 'every-string' | 'every-other-string', minFret: number) {
    ...
    expect(run.down[run.down.length - 1]).toMatchObject({ string: run.up[0]!.string, fret: run.up[0]!.fret });
    for (const p of [...run.up, ...run.down]) expect(p.fret).toBeLessThanOrEqual(instrument.fretCount);
    return run;
  }

  it.each(GUITARS)('climbs the neck and comes back by another route (%s)', (_id, instrument) => {
  ...
  it.each(GUITARS)('shifts on every string without leaving the neck (%s)', (_id, instrument) => {
  ...
  it('marks the extra note on a string as the shift', () => {
    const run = horizontalRun({ instrument: STANDARD_GUITAR, keyMode: C_MAJOR, minFret: 3, shiftOn: 'every-other-string' })!;
    ...
    expect(run.up.slice(0, 4).map((p) => [p.fret, p.shift])).toEqual([
      [3, undefined], [5, undefined], [7, undefined], [8, 'up'],
    ]);
